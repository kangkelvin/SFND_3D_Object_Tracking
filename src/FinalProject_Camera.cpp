...
void loadCamConfig(cv::Mat &RT, cv::Mat &P_rect_00, cv::Mat &R_rect_00) {
...
int main(int argc, const char *argv[]) {
  vector<string> detectorTypes = {"FAST",      "BRISK",  "ORB", "AKAZE",
                                  "SHITOMASI", "HARRIS", "SIFT"};
  vector<string> descriptorTypes = {"BRISK", "SIFT", "BRIEF", "FREAK",
                                    "ORB"};  //  AKAZE
  for (auto detectorType : detectorTypes) {
    for (auto descriptorType : descriptorTypes) {
      ...
      loadCamConfig(RT, P_rect_00, R_rect_00);
      ...
      for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
        ...
        bVis = false;
        if (bVis)
          showLidarTopview(lidarPoints, cv::Size(4.0, 20.0),
                           cv::Size(1000, 1000));
        bVis = false;
        ...
        // shrinks each bounding box by the given percentage to avoid 3D
        ...
        if (bVis)
          show3DObjects((dataBuffer.end() - 1)->boundingBoxes,
                        cv::Size(4.0, 20.0), cv::Size(800, 1000), true);
        bVis = false;
        ...
        //     "BRIEF";  // BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
        ...
        if (dataBuffer.size() > 1) {
          ...
          for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin();
            ...
            for (auto it2 = (dataBuffer.end() - 2)->boundingBoxes.begin();
              ...
              if (it1->first == it2->boxID)  // check wether current match
                                             // partner corresponds to this BB
              {
                prevBB = &(*it2);
              }
            ...
            if (currBB->lidarPoints.size() > 0 &&
              ...
              t = 1000 * t / 1.0;
              cout << "IMG processing took: " << t << " ms\n";

              bool writeResult = false;
              if (writeResult) {
                string fullDir = "../dat/experiment-output/output.txt";
              ...
              if (bVis) {
                ...
                string outputDir = "../dat/experiment-output/" + detectorType +
                                   "_" + descriptorType + "_" +
                                   to_string(imgIndex) + ".jpg";
                bool bWrite = false;
                if (bWrite) {
                  cv::imwrite(outputDir, matchImg);
                } else {
    ...
    }    // eof loop over all descriptorType
  }      // eof loop over all detectorType
